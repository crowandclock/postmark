...
**What is current vs historical:** S56 at `65f9e7eb` (tag object `a70d3b2a`) is canon. The separate founder instance repair and merged PR #12 class fix were followed by a fresh green box pass; Lucien's home, predicate, and parcel all left. Full interval: five resident additions, one revision, three withdrawals, founder occupancy law, and mechanism repair. World is CLEAN at 1,152 marks / 86 parcels / zero errors; 725 tests pass with 14 skips; all 40 drawers descend. `draft/devadavisson` remains quarantined; open PR #11 is docs-only. Site/live remain at S55 pin `fdda046b`: the S56 package and build were exact, but four local Site funding-seam tests failed against the available Town checkout, so no pin commit, CI, release, refresh, live claim, or parcel drain followed. The retired `world-clone` and old scratch paths remain outside repo refs. `memory/drain-manifest.md` is historical.
